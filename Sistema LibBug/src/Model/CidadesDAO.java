...
public class CidadesDAO {
    ...
    public ArrayList<CidadesBEAN> listaCidades(String query) {
        ...
        try {
            while (rs.next()) {
                lista.add(new CidadesBEAN(rs.getInt("codigoCidade"), rs.getString("nomeCidade")));
            }
            rs.close();
    ...
    public CidadesBEAN findCidadeCodigo(int codigoCidade) {
        ...
        try {
            if (rs.next()) {
                result = new CidadesBEAN(rs.getInt("codigoCidade"), rs.getString("nomeCidade"));
            }
            rs.close();
        } catch (SQLException e) {
            e.printStackTrace();
     ...
     public ArrayList<CidadesBEAN> findCidadeNome (String cidade) {
        ...
        ResultSet rs = null;
        rs = MySQLDAO.getResultSet("SELECT * FROM CIDADE WHERE nomeCidade like ?", "%" +cidade+ "%");
        try {
            while (rs.next()) {
                lista.add( new CidadesBEAN(rs.getInt("codigoCidade"), rs.getString("nomeCidade")));
            }
    ...
    public int findId(CidadesBEAN cidade) {
        int result = 0;
        ResultSet rs = null;
        rs = MySQLDAO.getResultSet("SELECT * FROM CIDADE WHERE nomeCidade= ?",
                cidade.getNome());
        try {
            if (rs.next()) {
